...
const services = [
  ...
  { icon: Wrench, title: "Aspiratör & Fan Sistemleri", description: "Hücreli aspiratörler, eksenel ve radyal fanlar, tam donanımlı montaj ve devreye alma hizmetleri." },
...
export default function Services() {
  return (
    <section id="hizmetler" className="w-full flex flex-col items-center py-20 sm:py-32 bg-[#1e293b]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 w-full">
          {services.map(({ icon: Icon, title, description }, index) => (
            <motion.div 
              ...
              className="group flex flex-col h-full bg-[#334155] hover:bg-[#475569] p-6 sm:p-10 border border-[#475569] hover:border-[#0ea5e9]/50 rounded-sm transition-all duration-300 w-full shadow-sm overflow-hidden"
            >
              ...
              <div className="flex-1 flex flex-col mt-6 sm:mt-8 min-w-0">
                <h3 className="font-display font-bold text-lg sm:text-2xl text-[#f8fafc] uppercase tracking-wide leading-tight">{title}</h3>
                <p className="mt-2 sm:mt-4 text-sm sm:text-base text-[#cbd5e1] leading-relaxed">{description}</p>
              </div>
            </motion.div>
          ))}
        </div>
